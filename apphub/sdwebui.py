...
"""
    ...
    def installation(self, install_location):
        ...
        """
        self.cfg.install_location = install_location
        # 其他的参数，也像 install_location 来赋值
        # self.cfg.xxxx = xxxx

        # 用 self.logger 来打印信息到日志，并且展示给用户
        self.logger.info(f"install with config {self.cfg}")

        # 创建安装目录（就算没有安装文件也要创建）
        os.makedirs(self.cfg.install_location, exist_ok=True)
        ...
        # 安装完成后，保存配置，这里是一个标准流程，当安装好之后，在其他任何函数中我们都可以使用 self.cfg
        # 中保存的信息，类似于应用的持久化状态，这里的 self.cfg 的类型就是 SdwebuiOption
